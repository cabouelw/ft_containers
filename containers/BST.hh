...
/*   Updated: 2022/05/15 16:44:46 by cabouelw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class BBST
{
	...
	public:
		...
		Node	*inserting(T data, Node *node = nullptr, Node *parent = nullptr)
		{
			if (!parent)
				node = root;
			if (!node)
			...
			if (node->data == data)
				return (node);
			if (data < node->data)
				node->left = inserting(data, node->left, node);
			else
				node->right = inserting(data, node->right, node);
			update(node);
			return (balance(node));
		}
